var mtCookieName="mt_blog_user";var mtCookieDomain=".etoday.ru";var mtCookiePath="/";var mtCookieTimeout=9176000;function mtHide(id){var el=(typeof id=="string")?document.getElementById(id):id;if(el)el.style.display='none';}
...
function _updateComments(){var comments,jsonUrl;isLoading=true;_setCurrentCommentsContent(loadingIcon);_setCommentOffset();jsonUrl=["http://www.etoday.ru/cgi-bin/mt/com.cgi?__mode=comment_listing&direction=",direction,"&entry_id=",entryID,"&limit=",commentsPerPage,"&offset=",_getCommentOffset()].join('');if(!commentAnchor){commentAnchor="comments-content";}
...
function _refreshComments(commentData){_setCommentContent(commentData);_setCurrentComments();window.location.hash='reset';window.location.hash=commentAnchor;_toggleNavLinks();}})();function mtRelativeDate(ts,fds){var now=new Date();var ref=ts;var delta=Math.floor((now.getTime()-ref.getTime())/1000);var str;if(delta<60){str='только что';}else if(delta<=86400){var hours=Math.floor(delta/3600);var min=Math.floor((delta%3600)/60);if(hours==1)
str='1 час назад';else if(hours>1)
str='2 часа назад'.replace(/2/,hours);else if(min==1)
str='1 минуту назад';else
str='2 минуты назад'.replace(/2/,min);}else if(delta<=604800){var days=Math.floor(delta/86400);var hours=Math.floor((delta%86400)/3600);if(days==1)
str='1 день назад';else if(days>1)
str='2 дня назад'.replace(/2/,days);else if(hours==1)
str='1 час назад';else
str='2 часа назад'.replace(/2/,hours);}
return str?str:fds;}
function mtEditLink(entry_id,author_id){var u=mtGetUser();if(!u)return;if(!entry_id)return;if(!author_id)return;if(u.id!=author_id)return;var link='<a href="mt.cgi?__mode=view&amp;_type=entry&amp;id='+entry_id+'">Редактировать</a>';document.write(link);}
...
f.armor.value='fad3c9c5fd445fa0441d49cd4f4366c49009e921';if(f.bakecookie&&f.bakecookie.checked)
...
return true;}
return false;}
function mtCommentSessionVerify(app_user){var u=mtGetUser();var f=document['comments_form'];if(u&&app_user&&app_user.sid&&(u.sid==app_user.sid)){f.submit();}else{alert('Ваша сессия истекла. Пожалуйста, авторизуйтесь ещё раз.');mtClearUser();mtFireEvent('usersignin');}}
...
if(el)
el.innerHTML='Авторизация… <span class="status-indicator">&nbsp;</span>';mtClearUser();mtFetchUser('mtSetUserOrLogin');return false;}
function mtSetUserOrLogin(u){if(u&&u.is_authenticated){mtSetUser(u);}else{mtSignIn();}}
function mtSignOut(entry_id){mtClearUser();var doc_url=document.URL;doc_url=doc_url.replace(/#.+/,'');var url='http://www.etoday.ru/cgi-bin/mt/mt-cp.cgi?__mode=logout&static=0';if(is_preview){if(document['comments_form']){var entry_id=document['comments_form'].entry_id.value;url+='&entry_id='+entry_id;}else{url+='&return_url=http%3A%2F%2Fwww.etoday.ru%2F';}}else{url+='&return_url='+encodeURIComponent(doc_url);}
location.href=url;}
...
el=document.getElementById('comment-form-external-auth');if(!el)return;var eid=cf.entry_id;var entry_id;if(eid)entry_id=eid.value;var phrase;var u=mtGetUser();if(u&&u.is_authenticated){if(u.is_banned){phrase='У вас нет прав для комментирования в этом блоге. (\<a href=\"javas\cript:void(0);\" onclick=\"return mtSignOutOnClick();\"\>выйти\<\/a\>)';}else{var user_link;if(u.is_author){user_link=u.name+'.';user_link+='<span class="edit-prifile-link">(<a href="http://www.etoday.ru/cgi-bin/mt/mt-cp.cgi?__mode=edit&blog_id=1&return_to='+encodeURIComponent(location.href);user_link+='" title="Настройка аватара, имени и электронной почты">Настройка профиля</a> | ';}else{user_link=u.name+'.';user_link+='<span class="edit-prifile-link">(<a href="http://www.etoday.ru/cgi-bin/mt/mt-cp.cgi?__mode=edit&blog_id=1&return_to='+encodeURIComponent(location.href);user_link+='" title="Настройка аватара, имени и электронной почты">Настройка профиля</a> | ';}
phrase='Привет! Сегодня вы — __NAME__\<a href=\"javas\cript:void(0);\" onclick=\"return mtSignOutOnClick();\"\>Выйти\<\/a\>)\<\/span\>';phrase=phrase.replace(/__NAME__/,user_link);}}else{if(reg_reqd){phrase='\<a href=\"javas\cript:void(0)\" onclick=\"return mtSignInOnClick(\'comment-greeting\')\"\>Авторизуйтесь\<\/a\> или \<a href=\"http://www.etoday.ru/cgi-bin/mt/mt-cp.cgi?__mode=register&blog_id=1&return_to='+
encodeURIComponent(location.href)+
'\"\>зарегистрируйтесь\<\/a\>, чтобы прокомментировать запись. <br />Для авторизации подходит любой OpenID-логин, в том числе: LiveJournal, Яндекс, Google, Facebook.';}else{phrase='\<a href=\"javas\cript:void(0)\" onclick=\"return mtSignInOnClick(\'comment-greeting\')\"\>Авторизуйтесь\<\/a\> или \<a href=\"http://www.etoday.ru/cgi-bin/mt/mt-cp.cgi?__mode=register&blog_id=1&return_to='+
encodeURIComponent(location.href)+
'\"\>зарегистрируйтесь\<\/a\>, чтобы не заполнять эту форму. <br />Для авторизации подходит любой OpenID-логин, в том числе: LiveJournal, Яндекс, Google, Facebook.';}}
el.innerHTML=phrase;}
function mtReplyCommentOnClick(parent_id,author){mtShow('comment-form-reply');var checkbox=document.getElementById('comment-reply');var label=document.getElementById('comment-reply-label');var text=document.getElementById('comment-text');var reply_text='Ответ на комментарий от \<a href=\"#comment-__PARENT__\" onclick=\"location.href=this.href; return false\"\>__AUTHOR__\<\/a\>';reply_text=reply_text.replace(/__PARENT__/,parent_id);reply_text=reply_text.replace(/__AUTHOR__/,author);label.innerHTML=reply_text;checkbox.value=parent_id;checkbox.checked=true;try{text.focus();}catch(e){}
...
mtFetchUser();}
var __MTTBLINK__;var __MTTBID__;function obfuscator(coded,key,mode,path,hidden){shift=coded.length;link="";for(i=0;i<coded.length;i++){if(key.indexOf(coded.charAt(i))==-1){ltr=coded.charAt(i);link+=(ltr);}else{ltr=(key.indexOf(coded.charAt(i))-shift+key.length)%key.length;link+=(key.charAt(ltr));}}
if(mode=='hidden_input'){document.write('<input type="hidden" name="CCode" value="'+link+'" />');}else if(mode=='__MTTBLINK__'){__MTTBLINK__=path+link;if(hidden)return;document.write(link);}else if(mode=='__MTTBID__'){__MTTBID__=link;if(hidden)return;document.write(link);}}